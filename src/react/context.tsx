...
export const ShuttleProvider = ({
  ...
  const providerInterface = useMemo(() => {
    const mobileConnect = async ({
      ...
      const mobileProvider = mobileProviders.find((mobileProvider) => mobileProvider.id === mobileProviderId);
      if (!mobileProvider) {
    ...
    const disconnect = (filters?: { providerId?: string; chainId?: string }) => {
      internalStore.getWallets(filters).forEach((wallet) => {
        const provider =
          ...
          mobileProviders.find((mobileProvider) => mobileProvider.id === wallet.providerId);
        if (provider) {
    ...
    const simulate = async ({ messages, wallet }: { messages: TransactionMsg[]; wallet?: WalletConnection | null }) => {
      ...
      const provider =
        ...
        mobileProviders.find((mobileProvider) => mobileProvider.id === walletToUse.providerId);
